...
class Diffusion2D {
    public:
            ...
            : D_(D), L_(L), M_(M), Mtot(M_*M_), dt_(dt), m(M_ - 1)
        {
            ...
            rho_ = (double*) _mm_malloc(M_*M_* sizeof(double), 32);
            rho_mid = (double*) _mm_malloc(M_*M_ * sizeof(double), 32);
            initialize_density();
        }
        ~Diffusion2D()
        {
        ...
        void write_density(std::string const& filename) const
        {
            ...
            for (size_type i = 0; i < M_; ++i) {
                for (size_type j = 0; j < M_; ++j)
                    out_file << (i*dh_) << '\t' << (j*dh_) << '\t' << rho_[i*M_ + j] << "\n";
                out_file << "\n";
            }
            out_file.close();
        }
        ...
        void initialize_density()
        {
            ...
            for (size_type i = 0; i < m+1; i++) {
                for (size_type j = 0; j < m+1; j++) {
                    rho_[i*M_ + j] = sin(PI*i*dh_)*sin(PI*j*dh_);
                }
            }
        }
