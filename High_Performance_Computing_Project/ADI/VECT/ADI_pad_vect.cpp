...
public:
		...
		: D_(D), L_(L), M_(M), Mtot(M_*M_), dt_(dt), m(M_ - 1)
	{
		...
		//stencil factor
		k_ = dt_ * D_ / (dh_ * dh_);
		//allocation of density vectors

		k_2 = k_/2;
		//vector containing the diagonal elements of the A matrix
		kb = 1 + k_;
		kbd = 1 / kb;
		//vectors containing the sub/super-diagonal elements of the A matrix
		ka = -k_ / 2;
		kc = -k_ / 2;
		nip = M_ - 2;
		...
		//auxiliary vector
		k_ = 1 - k_;
		//vectors for LHS of the linear systems
		b = (double*) _mm_malloc(4*nip*sizeof(double), 32);
		c = new double[8*nip];
		//        c.resize(nip,0);
	...
	//////////////////////////////////////////////////////
	//function that computes a step of the ADI algorithm//
	//////////////////////////////////////////////////////
	void advance()
	{
		size_type n = nip - 1; // since we start from x0 (not x1)
		...
		for (j=4; j < m; j=j+4) {
			...
			for ( i = 1; i < m; i++) {
				__m256d rho1 = _mm256_loadu_pd(rho_ + i*M_padded + (j-4));
				__m256d rho2 = _mm256_load_pd(rho_ + i*M_padded + (j-3));
                                __m256d rho3 = _mm256_loadu_pd(rho_ + i*M_padded + (j-2));
				...
				__m256d temp = _mm256_fnmadd_pd(kap,rho2, b3);
				__m256d res = _mm256_mul_pd(temp, denomp);
				_mm256_store_pd(rho_mid + (i)*M_padded + j-3,res);
				_mm256_store_pd(b + 4*i-4,b3);
	
				//rho_mid[(i + 1)*M_padded + j-3 ] = (b[4*i  ] - ka*rho_mid[(i)*M_padded + j-3 ])*denom[i];
                        	//rho_mid[(i + 1)*M_padded + j-2 ] = (b[4*i+1] - ka*rho_mid[(i)*M_padded + j-2 ])*denom[i];
				//rho_mid[(i + 1)*M_padded + j-1 ] = (b[4*i+2] - ka*rho_mid[(i)*M_padded + j-1 ])*denom[i];
				//rho_mid[(i + 1)*M_padded + j   ] = (b[4*i+3] - ka*rho_mid[(i)*M_padded + j   ])*denom[i];
				/*b[4*i - 4] = k_2 *rho_[i*M_padded + (j - 4)] + ( k_)*rho_[i*M_padded + j-3] + k_2 * rho_[i*M_padded + (j - 2)];
				b[4*i - 3] = k_2 * rho_[i*M_padded + (j - 3)] + ( k_)*rho_[i*M_padded + j-2] + k_2 * rho_[i*M_padded + (j - 1)];
				b[4*i - 2] = k_2 * rho_[i*M_padded + (j - 2)] + ( k_)*rho_[i*M_padded + j-1] + k_2 * rho_[i*M_padded + (j )];
				b[4*i - 1] = k_2 * rho_[i*M_padded + (j - 1)] + ( k_)*rho_[i*M_padded + j] + k_2 * rho_[i*M_padded + (j+1)];
*/
			}
			...
			__m256d b2 = _mm256_load_pd(b + 4*nip -4 );
			__m256d res2 = _mm256_fmadd_pd(k2p,rho2,b2);
			_mm256_store_pd(b + 4*nip -4,res2);

				
			//b[4*nip - 4] = b[4*nip - 4] + k_2 * rho_[m*M_padded + j-3];
			//b[4*nip - 3] = b[4*nip - 3] + k_2 * rho_[m*M_padded + j-2];
			//b[4*nip - 2] = b[4*nip - 2] + k_2 * rho_[m*M_padded + j-1];
			//b[4*nip - 1] = b[4*nip - 1] + k_2 * rho_[m*M_padded + j  ];
			...
			__m256d b1 = _mm256_load_pd(b+4*nip-4);
				
			__m256d temp = _mm256_fnmadd_pd(kap,rho2, b1);
			__m256d res = _mm256_mul_pd(temp, denomp);
			_mm256_store_pd(rho_mid + (n + 1)*M_padded + j-3,res);

			//rho_mid[(n + 1)*M_padded + j - 3 ] = (b[4*nip-4] - ka*rho_mid[n*M_padded + j - 3]) *denom[n];
                        //rho_mid[(n + 1)*M_padded + j - 2 ] = (b[4*nip-3] - ka*rho_mid[n*M_padded + j - 2]) *denom[n];
                        //rho_mid[(n + 1)*M_padded + j - 1 ] = (b[4*nip-2] - ka*rho_mid[n*M_padded + j - 1]) *denom[n];
                        //rho_mid[(n + 1)*M_padded + j     ] = (b[4*nip-1] - ka*rho_mid[n*M_padded + j - 0]) *denom[n];
	...
	/////////////////////////////////////////////////////////////////////
	//function that writes the current density in an external file .dat//
	/////////////////////////////////////////////////////////////////////
...
private:
	...
	///////////////////////////////////////////////////////
	//function that initializes the density at time t = 0//
	///////////////////////////////////////////////////////
