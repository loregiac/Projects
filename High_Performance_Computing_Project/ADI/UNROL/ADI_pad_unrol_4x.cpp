        ...
        void advance()
        {
            ...
            for (j=4; j < m; j=j+4) {
                ...
                for ( i = 1; i < m; i++) {
                    ...
                    rho_mid[(i )*M_padded + j-3 ] = (b[4*i -4 ] - ka*rho_mid[(i-1)*M_padded + j-3 ])*denom[i-1];
                    rho_mid[(i )*M_padded + j-2 ] = (b[4*i-3] - ka*rho_mid[(i-1)*M_padded + j-2 ])*denom[i-1];
                    rho_mid[(i)*M_padded + j-1 ] = (b[4*i-2] - ka*rho_mid[(i-1)*M_padded + j-1 ])*denom[i-1];
                    rho_mid[(i )*M_padded + j ] = (b[4*i-1] - ka*rho_mid[(i-1)*M_padded + j ])*denom[i-1];
                }
