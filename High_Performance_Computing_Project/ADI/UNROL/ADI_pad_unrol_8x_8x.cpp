        ...
        void advance()
        {
            ...
            for (i = 8; i < m; i+=8) {
                ...
                for (size_type j = 1; j < m; ++j) {
                    c[j-1] = k_2*rho_mid[(i - 8)*M_padded + j] + (k_)*rho_mid[(i-7)*M_padded + j] + k_2 * rho_mid[(i - 6)*M_padded + j];
                    c[nip+j-1] = k_2*rho_mid[(i - 7)*M_padded + j] + (k_)*rho_mid[(i-6)*M_padded + j] + k_2 * rho_mid[(i - 5)*M_padded + j];
                    c[2* nip+ j-1] = k_2 * rho_mid[(i - 6)*M_padded + j] + (k_)*rho_mid[(i-5)*M_padded + j] + k_2 * rho_mid[(i-4 )*M_padded + j];
                    c[3* nip+j-1] = k_2 * rho_mid[(i - 5)*M_padded + j] + (k_)*rho_mid[(i-4)*M_padded + j] + k_2 * rho_mid[(i - 3)*M_padded + j];

                    c[4*nip+j-1] = k_2*rho_mid[(i - 4)*M_padded + j] + (k_)*rho_mid[(i-3)*M_padded + j] + k_2 * rho_mid[(i - 2)*M_padded + j];
                    c[5*nip+j-1] = k_2*rho_mid[(i - 3)*M_padded + j] + (k_)*rho_mid[(i-2)*M_padded + j] + k_2 * rho_mid[(i - 1)*M_padded + j];
                    c[6*nip+j-1] = k_2 * rho_mid[(i - 2)*M_padded + j] + (k_)*rho_mid[(i-1)*M_padded + j] + k_2 * rho_mid[(i )*M_padded + j];
                    c[7*nip+j-1] = k_2 * rho_mid[(i - 1)*M_padded + j] + (k_)*rho_mid[i*M_padded + j] + k_2 * rho_mid[(i + 1)*M_padded + j];

                    rho_[(i-7)*M_padded+j]= (c[j-1] - ka*rho_[(i-7)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-6)*M_padded+j]= (c[nip+j-1] - ka*rho_[(i-6)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-5)*M_padded+j]= (c[2*nip+j-1] - ka*rho_[(i-5)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-4)*M_padded+j]= (c[3*nip+j-1] - ka*rho_[(i-4)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-3)*M_padded+j]= (c[4*nip+j-1] - ka*rho_[(i-3)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-2)*M_padded+j]= (c[5*nip+j-1] - ka*rho_[(i-2)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-1)*M_padded+j]= (c[6*nip+j-1] - ka*rho_[(i-1)*M_padded+j-1]) *denom[j-1];
                    rho_[(i-0)*M_padded+j]= (c[7*nip+j-1] - ka*rho_[(i-0)*M_padded+j-1]) *denom[j-1];
                }

                c[nip - 1] += k_2 * rho_mid[(i-7)*M_padded + m];
                c[2*nip - 1] += k_2 * rho_mid[(i-6)*M_padded + m];
                c[3*nip - 1] += k_2 * rho_mid[(i-5)*M_padded + m];
                c[4*nip - 1] += k_2 * rho_mid[(i-4)*M_padded + m];

                c[5*nip - 1] += k_2 * rho_mid[(i-3)*M_padded + m];
                c[6*nip - 1] += k_2 * rho_mid[(i-2)*M_padded + m];
                c[7*nip - 1] += k_2 * rho_mid[(i-1)*M_padded + m];
                c[8*nip - 1] += k_2 * rho_mid[i*M_padded + m];


                rho_[(i-7)*M_padded+nip] = (c[n] - ka*rho_[(i-7)*M_padded+nip - 1]) *denom[n];
                rho_[(i-6)*M_padded+nip] = (c[nip+n] - ka*rho_[(i-6)*M_padded+ nip- 1]) *denom[n];
                rho_[(i-5)*M_padded+nip] = (c[2*nip+n] - ka*rho_[(i-5)*M_padded+nip-1]) *denom[n];
                rho_[(i-4)*M_padded+nip] = (c[3*nip+n] - ka*rho_[(i-4)*M_padded+nip - 1]) *denom[n];

                rho_[(i-3)*M_padded+nip] = (c[4*nip+n] - ka*rho_[(i-3)*M_padded+nip - 1]) *denom[n];
                rho_[(i-2)*M_padded+nip] = (c[5*nip+n] - ka*rho_[(i-2)*M_padded+ nip- 1]) *denom[n];
                rho_[(i-1)*M_padded+nip] = (c[6*nip+n] - ka*rho_[(i-1)*M_padded+nip-1]) *denom[n];
                rho_[(i)*M_padded+nip] = (c[7*nip+n] - ka*rho_[i*M_padded+nip - 1]) *denom[n];
