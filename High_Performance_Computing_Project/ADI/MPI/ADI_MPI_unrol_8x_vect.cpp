...
# 198 "ADI_MPI_unrol_8x_vect.cpp"
            ...
            for (i = 9; i < M_block; i+=8) {

                rho_mid[(i-7)*M_padded] = 0;
                rho_mid[(i-6)*M_padded] = 0;
                rho_mid[(i-5)*M_padded] = 0;
                rho_mid[(i-4)*M_padded] = 0;
                rho_mid[(i-3)*M_padded] = 0;
                rho_mid[(i-2)*M_padded] = 0;
                rho_mid[(i-1)*M_padded] = 0;
                rho_mid[(i-0)*M_padded] = 0;
                for (size_type j = 1; j < m; ++j) {
                    c[8*j-8] = k_2*rho_[(i - 8)*M_padded + j] + (k_)*rho_[(i-7)*M_padded + j] + k_2 * rho_[(i - 6)*M_padded + j];
                    c[8*j-7] = k_2*rho_[(i - 7)*M_padded + j] + (k_)*rho_[(i-6)*M_padded + j] + k_2 * rho_[(i - 5)*M_padded + j];
                    c[8*j-6] = k_2 * rho_[(i - 6)*M_padded + j] + (k_)*rho_[(i-5)*M_padded + j] + k_2 * rho_[(i-4 )*M_padded + j];
                    c[8*j-5] = k_2 * rho_[(i - 5)*M_padded + j] + (k_)*rho_[(i-4)*M_padded + j] + k_2 * rho_[(i - 3)*M_padded + j];

                    c[8*j-4] = k_2*rho_[(i - 4)*M_padded + j] + (k_)*rho_[(i-3)*M_padded + j] + k_2 * rho_[(i - 2)*M_padded + j];
                    c[8*j-3] = k_2*rho_[(i - 3)*M_padded + j] + (k_)*rho_[(i-2)*M_padded + j] + k_2 * rho_[(i - 1)*M_padded + j];
                    c[8*j-2] = k_2 * rho_[(i - 2)*M_padded + j] + (k_)*rho_[(i-1)*M_padded + j] + k_2 * rho_[(i )*M_padded + j];
                    c[8*j-1] = k_2 * rho_[(i - 1)*M_padded + j] + (k_)*rho_[i*M_padded + j] + k_2 * rho_[(i + 1)*M_padded + j];

                    rho_mid[(i-7)*M_padded+j]= (c[8*j-8] - ka*rho_mid[(i-7)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-6)*M_padded+j]= (c[8*j-7] - ka*rho_mid[(i-6)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-5)*M_padded+j]= (c[8*j-6] - ka*rho_mid[(i-5)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-4)*M_padded+j]= (c[8*j-5] - ka*rho_mid[(i-4)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-3)*M_padded+j]= (c[8*j-4] - ka*rho_mid[(i-3)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-2)*M_padded+j]= (c[8*j-3] - ka*rho_mid[(i-2)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-1)*M_padded+j]= (c[8*j-2] - ka*rho_mid[(i-1)*M_padded+j-1]) *denom[j-1];
                    rho_mid[(i-0)*M_padded+j]= (c[8*j-1] - ka*rho_mid[(i-0)*M_padded+j-1]) *denom[j-1];
                }
            ...
            for (; i < M_block; ++i) {
                ...
                c[nip - 1] = c[nip - 1] + k_2 * rho_[i*M_padded + m ];
            ...
            for (j=9; j < M_block; j=j+8) {

                __m256d rho_recv1 = _mm256_set1_pd(0);
                __m256d rho_recv2 = _mm256_set1_pd(0);
                _mm256_storeu_pd(rho_mid + j - 7, rho_recv1);
                _mm256_storeu_pd(rho_mid + j - 3, rho_recv2);

                __m256d k2p = _mm256_broadcast_sd(&k_2);
                __m256d kp = _mm256_broadcast_sd(&k_);
                __m256d kap = _mm256_broadcast_sd(&ka);
                for ( size_t i = 1; i < m; i++) {
                    __m256d rho1 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-8));
                    __m256d rho2 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-7));
                    __m256d rho3 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-6));
                    __m256d b1 = _mm256_mul_pd(k2p,rho1);
                    __m256d b2 = _mm256_fmadd_pd(kp,rho2,b1);
                    __m256d b3 = _mm256_fmadd_pd(k2p,rho3,b2);
                    _mm256_storeu_pd(b + 8*i-8,b3);

                    __m256d rho4 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-4));
                    __m256d rho5 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-3));
                    __m256d rho6 = _mm256_loadu_pd(rho_recv + i*M_padded + (j-2));
                    __m256d b4 = _mm256_mul_pd(k2p,rho4);
                    __m256d b5 = _mm256_fmadd_pd(kp,rho5,b4);
                    __m256d b6 = _mm256_fmadd_pd(k2p,rho6,b5);
                    _mm256_storeu_pd(b + 8*i-4,b6);



                    __m256d rho7 = _mm256_loadu_pd(rho_mid + (i-1)*M_padded + (j-7));
                    __m256d rho8 = _mm256_loadu_pd(rho_mid + (i-1)*M_padded + (j-3));
                    ...
                    _mm256_storeu_pd(rho_mid + (i)*M_padded + j-7,res);



                    __m256d temp2 = _mm256_fnmadd_pd(kap,rho8, b6);
                    __m256d res2 = _mm256_mul_pd(temp2, denomp);
                    _mm256_storeu_pd(rho_mid + (i )*M_padded + j-3,res2);



                }
# 407 "ADI_MPI_unrol_8x_vect.cpp"
                __m256d rho3 = _mm256_loadu_pd(rho_mid + n*M_padded + (j-7));
                __m256d denomp = _mm256_broadcast_sd(denom+n);
                __m256d b3 = _mm256_loadu_pd(b+8*nip-8);

                __m256d temp1 = _mm256_fnmadd_pd(kap,rho3, b3);
                __m256d res3 = _mm256_mul_pd(temp1, denomp);
                _mm256_storeu_pd(rho_mid + (n + 1)*M_padded + j-7,res3);


                __m256d rho4 = _mm256_loadu_pd(rho_mid + n*M_padded + (j-3));
                __m256d b4 = _mm256_loadu_pd(b+8*nip-4);

                __m256d temp2 = _mm256_fnmadd_pd(kap,rho4, b4);
                __m256d res4 = _mm256_mul_pd(temp2, denomp);
                _mm256_storeu_pd(rho_mid + (n + 1)*M_padded + j-3,res4);
                ...
                for (size_t i = n; i-- > 0;) {
                    __m256d rho2 = _mm256_loadu_pd(rho_mid + (i+1)*M_padded + (j-7));
                    ...
                    _mm256_storeu_pd(rho_mid + (i + 1)*M_padded + j-7,res);
    ...
    private:
        ...
        double D_, L_;
        size_type M_, Mtot, m, nip, Mx_loc,My_loc, M_glob, M_block, j, i;
        size_type M_padded;
