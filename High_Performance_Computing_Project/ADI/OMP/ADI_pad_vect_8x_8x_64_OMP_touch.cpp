...
class Diffusion2D {
    public:
            ...
            : D_(D), L_(L), M_(M), Mtot(M_*M_), dt_(dt), m(M_ - 1)
        {
            ...
            rho_ = (double*) malloc((M_*M_padded+3)* sizeof(double));
            rho_mid = (double*) malloc((M_*M_padded+3) * sizeof(double));
...
#pragma omp for nowait
            for (size_type j=8; j < m; j=j+8) {
                ...
                for ( size_type i = 1; i < m; i++) {
                    ...
                    __m256d rho2 = _mm256_loadu_pd(rho_ + i*M_padded + (j-7));
                    ...
                    __m256d rho5 = _mm256_loadu_pd(rho_ + i*M_padded + (j-3));
                ...
                __m256d rho1 = _mm256_loadu_pd(rho_ + m*M_padded + j-7);
                __m256d b1 = _mm256_loadu_pd(b + 8*nip -8 );
                __m256d res1 = _mm256_fmadd_pd(k2p,rho1,b1);
                _mm256_storeu_pd(b + 8*nip -8,res1);

                __m256d rho2 = _mm256_loadu_pd(rho_ + m*M_padded + j-3);
                __m256d b2 = _mm256_loadu_pd(b + 8*nip -4 );
                __m256d res2 = _mm256_fmadd_pd(k2p,rho2,b2);
                _mm256_storeu_pd(b + 8*nip -4,res2);
...
#pragma omp for schedule(dynamic)
            for(size_type j=m-m%8-8*(m%8==0)+1; j < m; j++){
                for ( size_type i = 1; i < m; i++) {
...
#pragma omp for schedule (dynamic)
            for (size_type i=m-m%8-8*(m%8==0)+1; i < m; ++i) {
...
#pragma omp single
        ...
        double *b = (double*) malloc(8*(M-2)*sizeof(double));
        double *c = (double*) malloc(8*(M-2)*sizeof(double));
